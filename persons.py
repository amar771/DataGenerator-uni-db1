...
def GetRandomDate(lowYear=2014, highYear=2015):
...
# Formats date in Access accpetable format of:
# m/d/yyyy 0:00:00
...
# Clears file if they exist in case of need to regenerate data
...
def WriteToFileClients(loginID, registrationDate):
    ...
    client = str(loginID) + ','
    client += ccn + ','
    client += GetDateFormatted(GetRandomDate(2018, 2025)) + ','
...
def WriteToFilePersons(start, end):
    ...
    for i in range(start, end):
        ...
        line = str(i) + ','
        line += str(random.randint(1, 3061831)) + ','
        line += name + ','
        line += surname + ','
